...
import { CreateTestUser, LoginTestUser } from "./user.test";
...
export const TestPost: PostSchema & { fileName?: string } = {
    title: "Hello !",
    body: "This is a test",
...
export async function CreateTestPost(app: Application, user: UserCredentials, post: PostSchema & { fileName?: string }) {
    const resLogin = await LoginTestUser(app, user);

    authToken = "Bearer " + resLogin.body.token;

    let req = chai.request(app).post(Routes.posts).set("Authorization", authToken).field("title", post.title).field("body", post.body)

    for (let i = 0; i < post.authors.length; i++) {
        req = req.field("authors[" + i + "]", post.authors[i].toString())
    }

    if (post.fileName !== undefined) req = req.attach('file', path.resolve(__dirname, post.fileName))
...
describe("Posts", () => {
    ...
    beforeEach(async () => {
        ...
        TestPost.authors = new Array(authId)
    });
