...
class SnippetManager {
    constructor({ installdir, instanceState }) {
        ...
        instanceState.shellApi.snippet = instanceState.context.snippet = wrapperFn;
    ...
    async prepareIndex(refreshMode = 'allow-cached') {
        ...
        return this.repos = await ((_a = this.inflightFetchIndexPromise) !== null && _a !== void 0 ? _a : (this.inflightFetchIndexPromise = (async () => {
            ...
            await fs_1.promises.mkdir(this.installdir, { recursive: true });
            const sourceURLs = (await this.config.get('snippetIndexSourceURLs')).split(';').filter(Boolean);
            let repoData = null;
    ...
    async execFile([cmd, ...args]) {
        ...
        interrupted.checkpoint();
        const proc = cross_spawn_1.default(cmd, args, {
            cwd: this.installdir,
        ...
        switch (args[0]) {
            ...
            case 'ls': {
                ...
                for (const { name, snippetName } of this.snippets) {
                    packages = packages.replace(new RegExp(escape_string_regexp_1.default(name), 'g'), `mongosh:${snippetName}`);
                }
