...
async function resolveDnsSrvRecord(dns, lookupAddress) {
    const addresses = await util_1.promisify(dns.resolveSrv)(`_mongodb._tcp.${lookupAddress}`);
...
async function resolveDnsTxtRecord(dns, lookupAddress) {
    ...
    let records;
    try {
        records = await util_1.promisify(dns.resolveTxt)(lookupAddress);
    }
    catch (err) {
